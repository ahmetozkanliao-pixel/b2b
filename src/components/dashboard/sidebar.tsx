...
export function Sidebar({ role, userName, userEmail, companyName }: SidebarProps) {
  ...
  return (
    <aside className="panel-sidebar sticky top-14 hidden h-[calc(100vh-3.5rem)] w-60 shrink-0 flex-col border-r lg:flex">
      <div className="flex h-14 items-center border-b border-white/10 px-4">
        <Logo size="sm" showText={false} variant="light" />
        <span className="ml-2 text-sm font-medium text-slate-300">{t("common.panel")}</span>
      </div>
      ...
      {userName && (
        <Link
          ...
          className="mx-3 mt-3 flex items-center gap-3 rounded-lg border border-white/10 bg-white/5 p-3 transition-colors hover:border-brand-400/30 hover:bg-brand-500/10"
        >
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md gradient-brand text-xs font-semibold text-white shadow-soft">
      ...
      <nav className="flex-1 space-y-0.5 p-3">
        {links.map((link) => {
          ...
          return (
            ...
            >
              <link.icon className={cn("h-4 w-4 shrink-0", isActive && "text-brand-300")} />
              {t(link.labelKey)}
            </Link>
          );
        })}
